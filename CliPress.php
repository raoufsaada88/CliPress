...
class CliPress {
    ...
    public function render_admin_page() {
        ...
        if (isset($_GET['output']) && current_user_can('manage_options')) {
            $output_raw = base64_decode(sanitize_text_field(wp_unslash($_GET['output'])));
            echo '<h2>' . esc_html__('Command Output:', 'clipress') . '</h2>';
            echo '<pre id="clipress-command-output">' . esc_html($output_raw) . '</pre>';
        }
        ...
        if (!empty($logs)) {
            ...
            foreach ($recent_logs as $log) {
                ...
                printf(
                    ...
                    '<td><code>' . esc_html($command) . '</code></td>' .
                    '<td><pre style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">' . esc_html($output) . '</pre></td>',
                    '</tr>',
                    "\n"
                );
            }
    ...
    public function run_command() {
        ...
        $command_raw = isset($_POST['command']) ? wp_unslash($_POST['command']) : '';
        $command = sanitize_text_field($command_raw);
